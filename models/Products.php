...
class Products extends model {
	...
	public function getListOfColors($filters = array()) {
		$array = array();
		$where = $this->buildWhere($filters);
		$sql  = "SELECT
			id_color,
	...
	public function getListOfSizes($filters = array()) {
		$array = array();
		$where = $this->buildWhere($filters);
		$sql  = "SELECT
			id_size,
	...
	public function getList($offset = 0, $limit = 6, $filters = array(), $random = false) {
		...
		$sql = "SELECT
			*,
			( select colors.name from colors where colors.id = products.id_color ) as color_name,
			( select sizes.name from sizes where sizes.id = products.id_size ) as size_name,
	...
	private function buildWhere($filters) {
		...
		if(!empty($filters['color'])) {
			$where[] = "id_color IN ('".implode("','", $filters['color'])."')";
		}
		if(!empty($filters['size'])) {
			$where[] = "id_size IN ('".implode("','", $filters['size'])."')";
		}
...
public function getOptionsByProductId($id) {
		...
		if($sql->rowCount() > 0) {
			...
			// Juntando tudo em um único array.
